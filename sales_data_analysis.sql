...
select * from sales_data where ProductName not in ('Headphones','Smartwatch') and region not in ('South');
...
select productName , sum(SaleAmount) as total_sales 
from sales_data 
group by productName
having total_sales>10000;

-- 10. Determine the average sale amount for 'Laptop' and 'Tablet' in each region, including only regions with an average above $800, treating NULL regions as 'Unspecified'.
select  coalesce(region,'Unspecified') , avg(SaleAmount) as avg_sales 
from sales_data 
where productName in ('Laptop' ,'Tablet')
group by region
having avg_sales>800;

-- 11. Identify sales representatives who have sold a total quantity of more than 50 units across all products

select SalesRepID ,sum(Quantity) as total_quantity 
from sales_data 
group by SalesRepID
...
select region,max(SaleAmount) as max_sales 
from sales_data
group by region
having max_sales >500;
...
select ProductName, CustomerName, concat(upper(ProductName),' ' ,lower(CustomerName)) as CustomerName_ProdctName
from sales_data;
...
select replace(CustomerName,' ','-') as newCustomerName from sales_data
order by newCustomerName;

-- 18. Select records where the length of the ProductName is greater than the length of the CustomerName.
select * from sales_data 
where length(ProductName)> length(CustomerName);
...
select  * from sales_data where SalesRepID is null;

-- 21.  Update the SalesRepID to '999' for all records where the SalesRepID is NULL
update sales_data set SalesRepID='999' where SalesRepID is NULL;
...
alter table sales_data add column DiscountPercent decimal(5,2) default '5.00';
alter table sales_data drop column DiscountPercent;

-- 23. Rename the CustomerName column to ClientName.
alter table sales_data rename column CustomerName to ClientName;
